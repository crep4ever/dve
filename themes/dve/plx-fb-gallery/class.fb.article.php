...
class Article
{
  ...
  public function setDescription($p_value)
  {
    if (empty($p_value))
    {
      $this->m_fields["chapo"] .= "<p>&nbsp;</p>";
    }
    $this->m_fields["chapo"] .= $p_value;
  }

  public function setUrl($p_value, $p_photo_count=0)
  {
    $html  = "<p><a href='$p_value'>";
    $html .= "Voir l'album photo complet sur Facebook";
    if ($p_photo_count > 0)
    {
      $html .= "&nbsp; ($p_photo_count photos)</a></p>";
    }
    $html .= "</a></p>";

    $this->m_fields["content"] .= $html;
  }
